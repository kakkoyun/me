...
out=$(
  LIVE_PROBE_CMD="$TMP/probe-by-marker.sh" run <<EOF
...
PROBE_LOG="$TMP/urls3.log"
: >"$PROBE_LOG"
out=$(
  LIVE_PROBE_CMD="$TMP/probe-record.sh" PROBE_LOG="$PROBE_LOG" run <<EOF
