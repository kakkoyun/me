...
reset
write_post ok.md "${VALID[@]}"
assert_rc "accepts a well-formed post" "0" "$(rc_of)"

reset
write_post nopub.md 'title: "T"' 'date: 2026-08-21T00:00:00Z'
assert_rc "rejects a post with no publishDate" "1" "$(rc_of)"

reset
write_post nodate.md 'title: "T"' 'publishDate: 2026-08-21T00:00:00Z'
assert_rc "rejects a post with no date" "1" "$(rc_of)"

reset
write_post baddate.md 'title: "T"' 'date: 2026-08-21T00:00:00Z' 'publishDate: August 21 2026'
assert_rc "rejects an unparseable publishDate" "1" "$(rc_of)"

reset
write_post dateonly.md 'title: "T"' 'date: 2026-08-21' 'publishDate: 2026-08-21'
assert_rc "accepts a bare YYYY-MM-DD publishDate" "0" "$(rc_of)"

reset
echo "no frontmatter here" >"$TMP/posts/raw.md"
...
reset
write_post ok.md "${VALID[@]}"
echo "---" >"$TMP/posts/_index.md"
assert_rc "ignores _index.md" "0" "$(rc_of)"

# ── promotedAt shape ─────────────────────────────────────────────────────────

reset
write_post p.md "${VALID[@]}" 'promotedAt:' '  - 2026-08-21T06:03:00Z'
assert_rc "accepts a valid promotedAt list" "0" "$(rc_of)"

reset
write_post p.md "${VALID[@]}" 'promotedAt:' '  - 2026-08-21T06:03:00Z' '  - 2026-09-04T06:02:00Z'
assert_rc "accepts multiple promotedAt entries" "0" "$(rc_of)"

reset
write_post p.md "${VALID[@]}" 'promotedAt:' '  - 2026-08-21'
assert_rc "rejects a promotedAt entry without a time component" "1" "$(rc_of)"

reset
write_post p.md "${VALID[@]}" 'promotedAt:'
