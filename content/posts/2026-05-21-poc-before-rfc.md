...
**Prototyping first is a good instinct.** Writing an RFC for something you don't fully understand is an exercise in confident fiction. You'll miss the hard cases, underspecify the edge conditions, and paper over the parts where the design breaks down — because you haven't tried to build it yet.
...
- Python's async libraries (asyncio, trio, anyio) make certain timeout and cancellation semantics that are natural in Java actively harmful.
...
Every engineering culture I've worked in claims RFCs should precede significant features. The problem is that "PoC first" gets treated as an exception — a pragmatic shortcut for building understanding — and then the exception quietly becomes the rule.
