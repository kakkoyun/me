...
Step one is genuinely useful. Trying to write a credible RFC for something you've never built usually produces creative fiction. The PoC tells you which corners of the design are sharp. The trouble is step three, where the things you wrote down to *learn* what the design could be turn into what the design *is*, mostly because nobody scheduled the meeting to decide otherwise.
...
Most SDK projects don't have either of these things. The flag, maybe, in the form of "experimental" labels that users routinely ignore. The vote, almost never.
...
Structurally, that's the same shape as the Rust nightly/stable split. It also reads to me as an acknowledgment that the original "let things stabilize through use" approach produced too many de-facto-stable-but-officially-experimental features that downstream implementers and instrumentation authors now have to keep supporting.

It's interesting to me that this correction only began in late 2025, six and a half years after the merger. The PoC-first instinct produced a usable observability standard faster than waiting for a complete spec would have. I don't think anyone regrets that. The cost is showing up now as a governance project to install the gate that Rust had from the start.
...
[^7]: [Simon Willison, "Writing code is cheap now" — Agentic Engineering Patterns (February 2026)](https://simonwillison.net/guides/agentic-engineering-patterns/code-is-cheap/). Both the cost drop and the caveat that while new code has "dropped in price to almost free," delivering *good* code "remains significantly more expensive than that."
